...
function createRequestClient(baseURL: string) {
  ...
  async function doReAuthenticate() {
    ...
    if (
      ...
      accessStore.loginMode === 'Escape2' || accessStore.loginMode === 'Escape'
  ...
  client.addRequestInterceptor({
    fulfilled: async (config) => {
      ...
      if (config.showmask == undefined || config.showmask == true) {
        loadingStore.open();
      }
  ...
  client.addResponseInterceptor<HttpResponse>({
    fulfilled: (response) => {
      const loadingStore = useLoadingStore();
      if (
        response.config.showmask == undefined ||
        response.config.showmask == true
      ) {
        loadingStore.close();
      }
  ...
  client.addResponseInterceptor(
    errorMessageResponseInterceptor((msg: string, error) => {
      const loadingStore = useLoadingStore();
      if (error.config.showmask == undefined || error.config.showmask == true) {
        loadingStore.close();
      }
