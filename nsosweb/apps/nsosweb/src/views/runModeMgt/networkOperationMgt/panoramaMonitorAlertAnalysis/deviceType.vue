...
async function queryData() {
  ...
  try {
    const res = await getStatisticsBySqType(queryForm);
    console.log('res', res);
    if (res) {
      data.tableData = res;
    }
  ...
  try {
    await downloadFile(
      import.meta.env.VITE_NSOSFS +
        '/monitorAlarmLog/statisticsBySqType/export',
      queryForm,
      '预警分析清单-设施类型.xlsx',
      'post',
    );
  ...
  <div>
    ...
    <div style="padding: 10px 20px 10px 20px">
      <el-table ref="tab" border stripe :data="data.tableData" height="680px">
        ...
        <el-table-column prop="sqType" align="center" label="设施类型">
          <template #default="scope">
            <span>{{ sqTypes[Number(scope.row.sqType) - 1] }}</span>
          </template>
        </el-table-column>
        <!-- <el-table-column
        ...
        <el-table-column align="center" label="最长持续时间">
          <template #default="scope">
            <span>{{ scope.row.maxKeepTime }}h</span>
          </template>
        </el-table-column>
        <el-table-column
