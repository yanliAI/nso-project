...
async function queryData() {
  ...
  try {
    const res = await getStatisticsByUnit(queryForm);
    if (res) {
      data.tableData = res;
    }
  } catch (error) {
...
async function downloadAttachment(row: any) {
  try {
    await downloadFile(
      import.meta.env.VITE_NSOSFS + '/monitorAlarmLog/statisticsByUnit/export',
      queryForm,
      '预警分析清单-运维单位.xlsx',
      'post',
    );
  ...
  <div>
    ...
    <div style="padding: 10px 20px 10px 20px">
      <el-table ref="tab" border stripe :data="data.tableData" height="680px">
        ...
        <el-table-column prop="unit" align="center" label="运维单位" />
        <el-table-column align="center" label="运维人员">
          <template #default="scope">
            <span style="color: #4C4E52;">管理员</span>
          </template>
        </el-table-column>
