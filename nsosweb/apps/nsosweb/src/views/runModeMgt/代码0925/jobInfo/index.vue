<template>
  <div style="padding: 0 20px; height: 100%" class="bg-white">
    <div>
      <el-card shadow="always" style="margin-bottom: 20px">
        <div style="margin-bottom: 20px">
          <div style="font-size: 20px; color: black; font-weight: 500; text-align: center">作业详情</div>
          <div style="text-align: end">
            <el-button type="info" @click="returnOrder" size="small">返回</el-button>
          </div>
        </div>
        <el-form ref="formRef" :inline="true" label-position="right" label-width="200px" :model="data">
          ...
          <el-row :gutter="80">
            ...
            <el-col :span="12">
              <el-form-item label="关联工作计划:" required style="width: 100%">
                <el-input v-model="data.workPlanOid" :disabled="true" />
              </el-form-item>
            </el-col>
          </el-row>
          <el-row :gutter="80">
          ...
          <el-row :gutter="80">
            <el-col :span="24">
              <el-form-item label="关联工单:" style="width: 100%">
                <span style="color: #3184fc" v-if="data.otherOid && data.otherOid !== ''">{{ data.otherOid }}</span>
          ...
          <el-row :gutter="80">
            <el-col :span="24">
              <el-form-item label="涉及运行变更:" style="width: 100%" required>
                <el-switch v-model="data.runChange" inline-prompt active-color="#13ce66" inactive-color="#ff4949" :active-value="1" :inactive-value="0" active-text="是" inactive-text="否" />
              </el-form-item>
            </el-col>
          </el-row>
          <el-row :gutter="80">
          ...
          <el-row :gutter="80">
            <el-col :span="24">
              <el-form-item label="工作票签发人:" style="width: 100%" required>
                <el-input v-model="data.signerUser" :disabled="true" />
              </el-form-item>
            </el-col>
          </el-row>
          <el-row :gutter="80">
            <el-col :span="24">
              <el-form-item label="工作负责人(监护人):" style="width: 100%" required>
                <el-input v-model="data.workLeader" :disabled="true" />
              </el-form-item>
            </el-col>
          </el-row>
          <el-row :gutter="80">
          ...
          <el-row :gutter="80">
            <el-col :span="24">
              <el-form-item label="堡垒机账号:" style="width: 100%">
          ...
          <el-row :gutter="80">
            ...
            <el-col :span="8">
              <el-form-item label="停服总时长:" style="width: 100%">
                <span v-if="data.timeList">{{ calculateDuration(extractTimeRanges(data.timeList)[0], extractTimeRanges(data.timeList)[1]) }}</span>
                <span v-else>0小时0分钟</span>
              </el-form-item>
            </el-col>
            <el-col :span="8">
            ...
            <el-col :span="24">
              <el-form-item label="作业系统:" style="width: 100%">
                <el-table border stripe ref="tableRef" :data="data.operationPlansItemList" max-height="300">
                  ...
                  <el-table-column label="实例ID" prop="appId" align="center" />
            ...
            <el-col :span="24">
              <el-form-item label="关联信息系统清单:" style="width: 100%">
                <el-table border stripe :data="data.operationPlansAppList" max-height="300">
                  ...
                  <el-table-column label="业务负责人" prop="workUserName" align="center" />
                  <el-table-column label="业务负责人数认账号" prop="workUserId" align="center" />
                  <el-table-column label="影响开始时间" align="center" />
                  <el-table-column label="影响结束时间" align="center" />
                  <el-table-column label="影响用户数" prop="influenceCount" align="center" />
                </el-table>
              </el-form-item>
            </el-col>
            ...
            <el-col :span="24">
              <el-form-item label="作业安排:" style="width: 100%">
                <el-table border stripe :data="data.imWorkPlanList" max-height="300">
                  ...
                  <el-table-column label="对象类型" align="center" show-overflow-tooltip />
            ...
            <el-row :gutter="20">
              <el-col v-for="(file, index) in attachList" :key="file.id" :span="12" style="height: 45px; line-height: 45px; margin-bottom: 20px; display: flex; align-items: center; justify-content: space-between; color: black; font-size: 14px; font-weight: 400">
                ...
                <div style="width: 100px; background-color: #f6f6f8; height: 45px">
                  {{ formatFileSize(file.size) }} <el-icon color="grey" class="download-icon" @click="downloadAttach(file.name, file.fileId, file.iDate)"><Download /></el-icon>
                </div>
              </el-col>
            </el-row>
          </div>
          ...
          <el-row :gutter="20" style="padding: 0 20px 0 100px; color: 14px; line-height: 1.5">
            <el-col v-for="(file, index) in AttachmentTitleName" :key="file.id" :span="12" style="margin-bottom: 20px; font-size: 14px; display: flex; align-items: center; justify-content: space-between">
              <div style="width: 150px; text-align: right; padding-right: 15px">{{ file }}</div>
              <div class="file-name" :title="file" style="color: #a6aebe">暂无附件</div>
            </el-col>
          </el-row>
          ...
          <div style="padding-left: 100px">
            ...
            <el-row :gutter="20">
              <el-col v-for="(file, index) in attachList" :key="file.id" :span="12" style="height: 45px; line-height: 45px; margin-bottom: 20px; display: flex; align-items: center; justify-content: space-between; color: black; font-size: 14px; font-weight: 400">
                ...
                <div style="width: 100px; background-color: #f6f6f8; height: 45px">
                  {{ formatFileSize(file.size) }}<el-icon color="grey" class="download-icon"><Download /></el-icon>
                </div>
              </el-col>
            </el-row>
          </div>
      ...
      <TaskConfig v-if="data?.taskId && sourceType === '2'" :taskId="data.taskId" :oid="oid"></TaskConfig>
    </div>

    <!-- 作业台账清单 -->
    <el-dialog v-model="dialogVisible.jobLedgerList" title="作业台账清单" width="1500px">
      <el-table border stripe ref="tableRef" :data="data?.configItems" max-height="600">
        <el-table-column label="操作" align="center" width="120" fixed>
          <template #default="{ row, $index }">
            <el-button size="small" type="primary" @click="openJobLedgerDetail(row)"> 查看</el-button>
          </template>
        </el-table-column>
        <el-table-column label="所属信息系统名称" prop="ci_name" align="center" show-overflow-tooltip />
    ...
    <el-dialog v-model="dialogVisible.jobLedgerDetail" title="作业台账清单" width="1000px">
    ...
    <el-dialog v-model="dialogVisible.jobSysList" title="作业系统" width="1000px">
      <el-row :gutter="20" style="margin: 20px 0">
        <el-col :span="12">
          <div class="flex items-center">
            <div class="dialog-title">配置项名称:</div>
    ...
    <el-dialog v-model="dialogVisible.operatingTicket" title="操作票" width="1000px">
      <el-row :gutter="20" style="margin: 20px 0">
        <el-col :span="24">
          <div class="flex items-center">
    ...
    <el-dialog v-model="dialogVisible.riskInfo" title="风险评估详情" width="1700px">
        ...
        <el-tabs v-model="activeName" class="demo-tabs" @tab-click="handleClick">
          <el-tab-pane label="风险评估" name="first">
            ...
            <el-row>
              ...
              <el-col :span="6" class="box1">90</el-col>
            </el-row>
            <el-row>
              ...
              <el-col :span="6" class="box1">{{ data.riskAssessObj.bScore1 + data.riskAssessObj.bScore2 + data.riskAssessObj.bScore3 + data.riskAssessObj.bScore4 + data.riskAssessObj.bScore5 + data.riskAssessObj.bScore6 }}</el-col>
            </el-row>
            <el-row>
              <el-col :span="6" class="box1">自评风险值</el-col>
              <el-col :span="6" class="box1">{{ data.riskLevelSum }}</el-col>
              <el-col :span="6" class="box1">作业环境和时段风险值C</el-col>
              <el-col :span="6" class="box1">{{ data.riskAssessObj.cScore11 + data.riskAssessObj.cScore12 + data.riskAssessObj.cScore21 + data.riskAssessObj.cScore22 + data.riskAssessObj.cScore3 + data.riskAssessObj.cScore41 + data.riskAssessObj.cScore42 + data.riskAssessObj.cScore5 + +data.riskAssessObj.cScore6 }}</el-col>
            </el-row>
...
import { getworkPlanTaskDetailApi } from '#/api'
...
async function query() {
  try {
    ...
    data.value.configItems = JSON.parse(data.value.configItems)
    // 拼接风险值
    riskVal.value = '计算:' + getLabelByValue('riskLevel', data.value.riskLevel) + `(${data.value.riskLevelSum})`
  } catch (err) {}
}

// 下载附件
async function downloadAttach(name: string, fileId: string, iDate: string) {
  try {
    await downloadFile(import.meta.env.VITE_NSOSFS + `/ydCommon/downloadFile?fileId=${fileId}&iDate=${iDate}`, {}, name, 'get')
  } catch {}
}
...
/**
 * 获取停服总时长 从timeList字符串中提取两个时间段
 * @param {string} timeList 时间字符串，格式如："停服:2025-09-19 19:00:00～2025-09-19 23:00:00"
 * @returns {Array} 包含两个时间段的数组，格式：[startTime, endTime]
 */
function extractTimeRanges(timeList) {
  // 默认时间段（当timeList为空时使用）
  const defaultStartTime = '2025-09-19 19:00:00'
  const defaultEndTime = '2025-09-19 23:00:00'

  // 如果timeList为空或无效，返回默认时间段
  ...
  try {
    // 使用正则表达式提取时间段
    const timeMatch = timeList.match(/(\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2})～(\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2})/)
...
const AttachmentTitleName = ['现场勘测及安全技术交底单:', '拓扑图:', '发布功能清单:', '配置变量清单:', '发布测试报告(含功能、性能、安全测试):', '出厂功能、性能、安全测试报告(未上线投运系统)', '第三方功能、性能、安全测试报告(已上线投运系统)']
