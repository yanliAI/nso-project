    ...
    <el-card shadow="never" style="width: 100%">
      <el-row style="width: 100%" v-loading="loading" element-loading-text="压缩包解析中...">
        ...
        <el-col :span="18">
          ...
          <el-row>
            <el-col :span="16" style="height: 680px; overflow-y: auto;">
              <!-- <FilePreview ref="filePreviewRef" :fileData="previewFileData" isZipFile="true"></FilePreview> -->
              ...
              <WordPreview v-if="fileType === 'doc' || fileType === 'docx' || fileType === 'txt'" ref="wordRef" :keywordList="keywordList" :fileData="previewFileData" isZipFile="true" @change="updateFindMatch"></WordPreview>
              <PdfPreview v-if="fileType === 'pdf'" ref="pdfRef" :keywordList="keywordList" isZipFile="true" @change="updateFindMatch"></PdfPreview>
              <FilePreview v-show="typeContains(fileType, 'pngjpgjpeggifsvg')" ref="filePreviewRef" :fileData="previewFileData" isZipFile="true"></FilePreview>
            </el-col>
            <el-col :span="8" style="border-left: 1px solid #e4e4e7; padding: 10px">
                ...
                <el-table-column label="审查结果" align="center">
                  <template #default="scope">
                    <div v-if="scope.row.keyword" class="search-content">
                      ...
                      <div>
                        <button @click="findPrev(scope.row.keyword)" class="nav-btn">◀</button>
                        <button @click="findNext(scope.row.keyword)" class="nav-btn">▶</button>
                      </div>
                    </div>
              ...
              <el-table border stripe :data="reviewItemResult" height="300">
...
import { previewZipFileApi, getZipFilesApi,getKeywordsAndChaptersApi } from '#/api'
