        ...
        <el-row :gutter="30">
          ...
          <el-col :span="8">
            <el-form-item label="修改时间：" prop="updateTime">
              <el-input v-model="modalFormData.updateTime" clearable disabled />
            </el-form-item>
          </el-col>
        </el-row>
      </el-form>
...
import {
  queryFixedValueWhiteListApi,
  saveFixedValueWhiteListDataApi,
  deleteFixedValueWhiteListDataApi,
} from '#/api';
...
const saveData = async (row) => {
  ...
  try {
    let result = await saveFixedValueWhiteListDataApi(params);
    if (result) {
...
const deleteData = (row) => {
  ElMessageBox.confirm('确定要删除这条数据吗?', '提示', {
    ...
    .then(async () => {
      try {
        let result = await deleteFixedValueWhiteListDataApi([row.id]);
        if (result) {
...
const query = async () => {
  ...
  try {
    const result = await queryFixedValueWhiteListApi(params);
...
const exportData = async () => {
  try {
    ...
    await downloadFile(
      import.meta.env.VITE_NSOSFS + '/fixedManage/whiteList/export',
      params,
      '定值规则管理-白名单数据.xlsx',
    );
  } catch {}
