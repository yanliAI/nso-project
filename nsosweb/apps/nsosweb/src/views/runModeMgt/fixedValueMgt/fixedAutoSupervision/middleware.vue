        ...
        >
          ...
          >
            <template #default="{ row }">
              <span>{{ row.isCluster == 1 ? '是' : '否' }}</span>
            </template>
          </el-table-column>
          <el-table-column
            prop="isManagementNode"
            label="是否是管理节点"
            align="center"
            width="110"
          >
            <template #default="{ row }">
              <span>{{ row.isManagementNode == 1 ? '是' : '否' }}</span>
            </template>
          </el-table-column>
          <el-table-column
    ...
    >
      ...
      >
        <el-row :gutter="30">
          ...
          <el-col :span="8">
            <el-form-item label="是否是集群" prop="isCluster">
              <el-switch
                v-model="modalFormData.isCluster"
                style="--el-switch-on-color: #13ce66"
                active-value="1"
                inactive-value="0"
              />
          ...
          <el-col :span="8">
            <el-form-item label="是否是管理节点" prop="isManagementNode">
              <el-switch
                v-model="modalFormData.isManagementNode"
                style="--el-switch-on-color: #13ce66"
                active-value="1"
                inactive-value="0"
              />
...
import {
  queryFixedValueMiddlewareListApi,
  saveFixedValueMiddlewareDataApi,
...
const lookSystemDetail = async (row) => {
  ...
  modalApi.open();
  try {
    let result = await getMiddlewareDataApi(row.id);
...
const saveData = async (row) => {
  let params = { ...row };
  try {
    let result = await saveFixedValueMiddlewareDataApi(params);
    if (result) {
...
const query = async () => {
  ...
  try {
    const result = await queryFixedValueMiddlewareListApi(params);
...
const exportData = async () => {
  try {
    ...
    await downloadFile(
      import.meta.env.VITE_NSOSFS + '/fixedAutoSupervision/middleware/export',
      params,
      '定值监督-中间件数据.xlsx',
    );
  } catch {}
