  ...
  <div class="app-container p-4">
    ...
    >
      <el-form
        ...
        label-width="auto"
      >
        <el-form-item label="用户组编码:" prop="id">
          <el-input
            style="width: 100%"
...
import {
  getFlowUserGroupListApi,
  saveOrUpdateFlowUserGroupApi,
  deleteFlowUserGroupApi,
  saveMembershipApi,
} from '#/api';
...
const addFormRules = ref({
  id: [{ required: true, message: '请输入用户组编码' }],
...
async function getList() {
  loading.value = true;
  try {
    let result = await getFlowUserGroupListApi(queryParams);
...
async function handleDelete(row) {
  ...
  ElMessageBox.confirm(tips, '提示', {
    ...
    .then(async () => {
      let result = await deleteFlowUserGroupApi(operIds);
      if (result) {
        getList();
...
const saveMembership = async () => {
  ...
  try {
    const result = await saveMembershipApi({
...
const saveOrUpdate = async () => {
  ...
  await addFormRef.value.validate(async (valid, fields) => {
    if (valid) {
      ...
      try {
        const result = await saveOrUpdateFlowUserGroupApi(params);
        if (result) {
          ElMessage.success('保存成功！');
          modalApi.close();
          getList();
