    ...
    <el-header height="90px">
      <el-form label-width="auto" ref="ruleFormRef" :model="data.formData" :inline="true" label-position="right" class="demo-form-inline">
        <el-row>
          ...
          <el-col :span="4">
            <!-- <el-form-item label="目录:" filterable>
              <el-select v-model="data.formData.saveDirectory" @focus="queryContent" clearable>
                <el-option v-for="item in contentList" :key="item" :value="item" />
              </el-select>
            </el-form-item> -->
            <el-form-item label="目录:" prop="saveDirectory">
          ...
          <el-col :span="8">
            <el-form-item label="创建时间:">
              <el-date-picker v-model="data.formData.createTime" type="datetimerange" value-format="YYYY-MM-DD HH:mm:ss" clearable />
            </el-form-item>
          </el-col>
          <el-col :span="4">
...
const query = async () => {
  ...
  data.formData.fileStatus = '1'

  if (data.formData.createTime != null) {
    data.formData.startCreateTime = data.formData.createTime[0]
    data.formData.endCreateTime = data.formData.createTime[1]
  }
