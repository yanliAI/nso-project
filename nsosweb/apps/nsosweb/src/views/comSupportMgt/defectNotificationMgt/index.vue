<template>
  <Page auto-content-height>
    <el-tabs type="border-card" ref="myTab" v-model="data.tabIndex" @tab-remove="removeTab">
      <el-tab-pane label="信息填报" name="list" :closable="false">
              ...
              <el-row>
                ...
                <el-col :span="4">
                  <el-form-item label="状态:" prop="effectRange">
                    <el-select v-model="data.formData.defectStatus" clearable>
      ...
      <el-tab-pane v-for="item in data.tabArr" :key="item.name" :label="item.title" :name="item.name" closable>
        <FaultDetail :faultId="item.faultId" :flag="'1'" @removeTab="removeTab" :name="item.name" @onSearch="onSearch"></FaultDetail>
      </el-tab-pane>
    </el-tabs>
  </Page>
</template>
...
import { toExcelTb } from './defectNotiMgtMethods.ts'
...
const batchDelete = () => {
  ...
  for (let i = 0; i < selectedList.length; i++) {
    if (selectedList[i].defectStatus !== '1') {
      ElMessage.warning('只能删除填报中的数据')
      return
    }
  }
...
const batchDeleteApi = async () => {
  try {
    await requestClient.post(import.meta.env.VITE_NSOSZH + '/defectNoticeInfo/deletebatchData', selectedRows.value)
    onSearch()
    selectedRows.value = []
...
const toExcelClick = async () => {
  toExcelTb(data.formData, '缺陷通知管理导出excel.xlsx')
}
