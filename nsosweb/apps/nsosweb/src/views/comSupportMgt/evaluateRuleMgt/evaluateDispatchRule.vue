<template>
  <div style="width: 100%; height: 100%">
              ...
              <el-col :span="5">
                <el-form-item label="评价项目:" prop="evaluateItemName">
                  <div class="primary truncate">
                    <el-tooltip :content="data.form.evaluateItemName" placement="bottom" effect="light">
                      {{ data.form.evaluateItemName }}
                    </el-tooltip>
                  </div>
                </el-form-item>
              </el-col>
        ...
        <el-main style="height: calc(100% - 50px)">
          <el-form ref="ruleFormRef" :model="data" :rules="formRules">
            <el-table ref="dispatchRuleTable" :data="data.tableData" border size="small" style="height: 100%" 
              ...
              <el-table-column prop="normalScore" label="标准分" align="center">
                <template #default="scope">
                  <el-form-item class="unedit_item" :prop="'tableData.' + scope.$index + '.normalScore'" :rules="formRules.normalScore">
                    <el-input v-if="data.editingRow === scope.row && data.editingCol === scope.column.property"  
                      v-model="scope.row.normalScore" @change="handleEditScore(scope.row)" type="number" :disabled="isScoreDisabled(scope.row)">
                    </el-input>
                    <span v-else>
              ...
              <!-- <el-table-column prop="rcOrder" label="排序号" align="center">
                <template #default="scope">
                  <el-form-item class="unedit_item" :prop="'tableData.' + scope.$index + '.rcOrder'" :rules="formRules.rcOrder">
                    <el-input v-if="data.editingRow === scope.row && data.editingCol === scope.column.property"  
                      v-model="scope.row.rcOrder" @change="handleEdit(scope.row)" type="number">
                    </el-input>
                    <span v-else>
                      {{ scope.row.rcOrder }}
                    </span>
              ...
              <el-table-column prop="uperEvaluateItemCode" label="上级评价项" column-key="NSOS_EVALUATE_DISPATCH_ITEM" :formatter="formatDrop"  align="center" />
            </el-table>
          </el-form>
        </el-main>
    ...
    <Modal class="w-[1000px] h-[380px]" title="运调工作评价发布" :footer="false" :closeOnClickModal="false" :draggable="true" headerClass="head-md">
      <DispatchRelease :busiType="busiType" @releaseCancel="releaseCancel"></DispatchRelease>
    </Modal>
  </div>
</template>
...
import DispatchRelease from './dispatchRelease.vue';

const busiType = defineModel('busiType');
const dropList: any = reactive({});

onMounted(async () => {
  const dropStore = useDropStore();
  const dropType = ['NSOS_EVALUATE_DISPATCH_ITEM'];
...
async function getFlowTableApi(params: queryParams) {
  return requestClient.post(
    import.meta.env.VITE_NSOSZH + '/evaluateDispatchRule/query',
    params,
    { params: {busiType: busiType.value} }
  );
}
...
async function getTreeDataApi() {
  return requestClient.get(
    import.meta.env.VITE_NSOSZH + '/evaluateDispatchRule/queryTree',
    { params: {busiType: busiType.value} }
  );
}
...
function handleEditScore(row, col) {
  if (!data.updateList.includes(row)) {
    data.updateList.push(row);
  }
  let targetNode = findNode(data.treeData, row.evaluateItemCode)
  if (targetNode) {
    targetNode.data.normalScore = Number(row.normalScore)
    data.treeData.forEach(node => {
      calculateTotalScore(node);
    });
    data.updateList = data.tableData
  }
}
...
function calculateTotalScore(node) {
  ...
  if (node.children && node.children.length > 0) {
    ...
    totalScore = node.data.normalScore;
  }
  node.data.normalScore = totalScore;
  let curData = data.tableData.filter(d => d.evaluateItemCode === node.code);
  curData[0].normalScore = totalScore;
  return totalScore;
}
...
async function saveDataApi(param: object) {
  return requestClient.post(
    import.meta.env.VITE_NSOSZH + '/evaluateDispatchRule/save',
    param,
    { params: {busiType: busiType.value} }
  );
}
...
async function duplicateValid() {
  ...
  // // 评价项编码 + 排序号
  // // 原数据 allTableData
  // // 新增修改 updateList 根据id区分
  // // 删除 delete
  // let itemCodeList = data.allTableData.map(data => data.evaluateItemCode);
  // let itemCodeAddList = data.updateList.filter(data => data.id == null).map(data => data.evaluateItemCode);
  // let itemCodeUpdateList = data.updateList.filter(data => data.id != null).map(data => data.evaluateItemCode);
  // let itemCodeDeleteList = data.deleteList.filter(data => data.id != null).map(data => data.evaluateItemCode);
  
  // // 处理删除
  // itemCodeList = itemCodeList.filter(data => !itemCodeDeleteList.includes(data));
  // // 处理修改
  // // 处理新增
