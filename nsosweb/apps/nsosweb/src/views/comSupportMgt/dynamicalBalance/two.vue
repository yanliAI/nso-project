  ...
  series: [
    {
      ...
      label: {
        ...
        formatter: (params) => {
          if (params.name == "物理机") {
            return "{a| " + params.value + "(MB)" + "}\n{a| " + params.percent + "%}";
          }
          else if (params.name == "虚拟机") {
            return "{b| " + params.value + "(MB)" + "}\n{b| " + params.percent + "%}";
          }
          else {
            return "{c|" + params.value + "(MB)" + "}\n{c| " + params.percent + "%}";
          }
        },
        rich: {
          a: {
...
const query = async () => {
  ...
  let sum = 0;
  response.forEach(function(item,index) {newAnnularList[index].value = item.bkMem; sum += item.bkMem;})
  option.title.subtext = sum + '(MB)';
