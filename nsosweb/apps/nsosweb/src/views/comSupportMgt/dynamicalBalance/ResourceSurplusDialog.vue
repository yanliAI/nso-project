<template>
  <el-dialog title="资源余量情况" :model-value="visible" width="80%" :before-close="handleClose" custom-class="custom-dialog">
    <div style="background-color: white;">
      ...
      <el-table border ref="workflowTab" :data="tableData" @selection-change="handleSelectionChange"
        ...
        <el-table-column prop="deviceModel" label="资源类型" column-key="NSOS_DEVICE_MODEL" :formatter="formatDrop" width="150">
        </el-table-column>
        <el-table-column prop="netSplit" label="网络分区" column-key="NSOS_NET_SPLIT" :formatter="formatDrop" width="150">
        </el-table-column>
        <el-table-column prop="bkCpu" label="CPU逻辑核心数(个)" width="250"></el-table-column>
        <el-table-column prop="bkMem" label="内存容量(MB)" ></el-table-column>
        <el-table-column prop="bkDisk" label="磁盘容量(GB)" width="250"></el-table-column>
      </el-table>
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const fetchTableData = async () => {
      // 模拟从服务器获取数据
      // 这里可以替换为实际的 API 请求
      const response = await requestClient.post(import.meta.env.VITE_NSOSZH + '/itResourceInfo/mHQueryDataListByPagePagination', {
    ...
    onMounted(async () => {
      ...
      const dropType = ['NSOS_DEVICE_MODEL', 'NSOS_NET_SPLIT']
