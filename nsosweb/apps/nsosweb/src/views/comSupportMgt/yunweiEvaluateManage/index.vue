  ...
  <Page auto-content-height>
    <el-tabs type="border-card" ref="myTab" v-model="data.tabIndex" @tab-remove="removeTab">
        ...
        <el-container class="h-full bg-white">
              ...
              <el-row>
                <el-col :span="6">
                  <el-form-item label="单位:" prop="evaluateUini">
                    <el-select v-model="data.formData.evaluateUini" type="month" clearable filterable>
                ...
                <el-col :span="10">
                  ...
                  <el-button type="info" plain class="ml-4" :icon="Histogram" @click="addAnalysis">排名分析</el-button>
                </el-col>
              </el-row>
              <el-row>
                <el-col :span="3">
                  <el-form-item label="评价维度:" prop="evaluateTimeType">
                    <el-select v-model="data.formData.evaluateTimeType" type="month" clearable filterable>
          ...
          <el-main style="height: calc(100% - 140px)">
            <el-table :data="data.tableData" border size="small" style="height: 100%" @selection-change="handleSelectionChange">
              ...
              <el-table-column prop="evaluateUini" label="单位" column-key="NSOS_ORG_NAME" :formatter="formatDrop" align="center" />
              ...
              <el-table-column prop="evaluateMonth" label="时间" align="center">
                <template #default="scope">
                  {{ scope.row.evaluteTimeType === '2' ? dayjs(new Date(scope.row.evaluateMonth)).format('YYYY') : dayjs(new Date(scope.row.evaluateMonth)).format('YYYY-MM') }}
                </template>
              </el-table-column>
      ...
      <el-tab-pane v-for="item in data.tabArr" :key="item.name" :label="item.title" :name="item.name" closable>
        <EvaluateDetail @removeTab="removeTab" :evaluateCode="item.evaluateCode" :evaluateStatus="item.evaluateStatus" :evaluateUini="item.evaluateUini" :name="item.name" @query="query"></EvaluateDetail>
      </el-tab-pane>
      <el-tab-pane v-for="item in data.tabAnalysis" :key="item.name" :label="item.title" :name="item.name" closable>
        <RankingAnalysis :evaluateTimeType="data.formData.evaluateTimeType" :evaluateMonth="data.formData.evaluateMonth" @removeTab="removeTab" :name="item.name" @query="query"></RankingAnalysis>
      </el-tab-pane>
    </el-tabs>
  </Page>
</template>
...
import EvaluateDetail from '../yunweiEvaluateManageDetail/index.vue'
