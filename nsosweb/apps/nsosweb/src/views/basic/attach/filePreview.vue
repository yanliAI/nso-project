...
<template>
  <div style="width: 100%; height: 100%">
      <div v-if="data.showLoading" class="load-d" v-loading="loading" element-loading-text="正在加载..." :element-loading-spinner="loadSvg" style=" width: 100%;height: 100%"></div>
      <iframe ref="pdfContainerRef" v-if="data.showType == 'pdf'" style=" width: 100%;height: 100%"></iframe>
      <div id="docContainer" v-if="data.showType == 'docx'"></div>
      <div v-if="typeContains(data.showType, 'pngjpgjpeggifsvg')" style=" display: flex; align-items: center;width: 100%; height: 100%">
          <el-image :src="data.imageSrc" :preview-src-list="data.imageSrcList"></el-image>
      </div>
      <div v-if="data.showType == 'txt'" v-html="data.txtHtml" style="width: 100%; height: 100%"></div>
      <!-- <div id="spContainer" v-if="fileType == 'mp4'" style="width: 100%; height: 100%"></div> -->
  </div>
</template>
...
onMounted(() => {
  ...
  axios({
      ...
      if (data.showType == "doc" || data.showType == "docx") {
        renderAsync(rs.data, document.getElementById("docContainer"));
      } else if (data.showType == "pdf") {
        pdfContainerRef.value.src = window.URL.createObjectURL(rs.data);
      } else if (data.showType == "txt") {
        const reader = new FileReader();
        reader.onload = () => {
            data.txtHtml = reader.result.replace(/\n|\r\n/g, "<br/>").replace(/ /g, " &nbsp"); // tab 换行 空格;
        };
        reader.readAsText(rs.data);
      } else if (typeContains(data.showType, 'pngjpgjpeggifsvg')) {
        data.imageSrc = window.URL.createObjectURL(rs.data)
        data.imageSrcList = [data.imageSrc]
      }
