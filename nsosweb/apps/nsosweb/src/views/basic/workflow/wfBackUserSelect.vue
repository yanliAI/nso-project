<template>
  <div class="w-full h-full">
    <div class="h-[50%] flex">
      ...
      <div class="gjasjg">
        <div class="flex text-sm pt-1 px-2" v-for="(item,index) in data.selectUser" :key="index">
          <div class="flex-[1]">{{item.userName}}({{item.deptPath}})[{{item.postName}}]</div>
    ...
    <div class="flex mx-[10px] mb-[10px]" style="height: calc(50% - 60px);">
      <el-input v-model="data.opinion" maxlength="1000" show-word-limit type="textarea" placeholder="请输入审批意见" clearable/>
    </div>
...
const nodeClick = (node) => {
  if (node.nodeInstanceId) {
    return
  }
  if (data.curNodeId == null) {
    data.curNodeId = node.nodeId
    data.selectUser.push(node)
  } else {
    if (data.curNodeId == node.nodeId) {
      if (!data.selectUser.includes(node)) data.selectUser.push(node)
    } else {
      ElMessage.warning('只能选择一个目标节点！');
      data.curNodeId = node.nodeId
      data.selectUser = []
      data.selectUser.push(node)
    }
...
const selectOkClick = async () => {
  ...
  newData = newData.filter(n => n.users.length > 0)
