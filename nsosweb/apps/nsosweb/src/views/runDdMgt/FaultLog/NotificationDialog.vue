<template>
  <el-dialog title="通知催办详情" :model-value="visible" width="70%" :before-close="handleClose" custom-class="custom-dialog">
    <div class="data-display">
      <span class="data-item">
        <span class="label">故障单号：</span>
        <span class="value blue-text">{{ formData.ticketNumber }}</span>
      </span>
      <span class="data-item">
        <span class="label">报告类型：</span>
        <span class="value blue-text">{{ getReportTypeLabel(formData.reportType) }}</span>
      </span>
      <span class="data-item">
        <span class="label">通知类型：</span>
        <el-select v-model="formData.notificationType" style="width: 150px" disabled>
            <el-option v-for="item in dropList.NSOS_RECORD_TYPE" :key="item.value"  :label="item.label" :value="item.value"/>
        </el-select>
      </span>
    </div>

    <h3 class="label">通知列表</h3>
    <el-table border :data="notificationList" style="width: 100%" @selection-change="handleSelectionChange" stripe :header-cell-style="{ backgroundColor: '#F2F3F5', color: '#666666', fontSize: '14px', height: '40px', textAlign: 'center' }">
      ...
      <el-table-column prop="reportType" label="报告类型" width="90" align="center">
        <template #default="scope">
          {{ getReportTypeLabel(scope.row.reportType) }}
        </template>
      </el-table-column>
      <el-table-column prop="recordType" label="通知类型" width="90" align="center">
        <template #default="scope">
          {{ gettzlxLabel(scope.row.recordType) }}
        </template>
      </el-table-column>
      <el-table-column prop="notifyMan" label="通知人" width="100" align="center"></el-table-column>
      <el-table-column prop="notifyTime" label="通知时间" :formatter="formatTimestamp" width="180" align="center"></el-table-column>
      <el-table-column prop="notifyComtent" label="联系内容" ></el-table-column>
      <el-table-column prop="notifyResult" label="通知结果" width="100" align="center">
        <template #default="scope">
      ...
      <el-table-column prop="notifyType" label="通知方式" width="100" align="center">
        <template #default="scope">
  ...
  setup(props, { emit }) {
    ...
    watch(
      ...
      (newVal) => {
        if (newVal) {
          // 更新 formData
          formData.ticketNumber = props.ticketNumber;
          formData.reportType = props.reportType;
          formData.notificationType = props.notificationType;
          fetchNotificationList();
        }
      }
    );
