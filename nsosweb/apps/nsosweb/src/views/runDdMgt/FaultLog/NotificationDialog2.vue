  ...
  <div style="width: 100%; height: 100%;">
    <el-container class="h-full bg-white">
      ...
      <el-form label-width="auto" ref="formRef" :model="formData" :rules="formRules" :inline="true"
        ...
        <el-row >
          <el-col :span="8">
              <el-form-item label="报告类型：" prop="reportType" style="margin-bottom: 18px">
                <el-select v-model="formData.reportType" clearable>
                <el-option v-for="item in dropList.NSOS_REPORT_TYPE" :key="item.value" :label="item.label" :value="item.value"/>
              </el-select>
              </el-form-item>
            </el-col>
          <el-col :span="8">
            <el-form-item label="故障单号:" prop="ticketNumber">
              <el-input v-model="formData.ticketNumber" clearable />
            </el-form-item>
          </el-col>
        <el-col :span="8">
              <el-form-item label="通知类型：" prop="notificationType" style="margin-bottom: 18px">
                <el-select v-model="formData.notificationType" clearable>
    ...
    <el-main style="height: calc(100% - 90px);">
      ...
      <el-table-column prop="reportType" label="报告类型" width="90" align="center" column-key="NSOS_REPORT_TYPE" :formatter="formatDrop"></el-table-column>
      <el-table-column prop="recordType" label="通知类型" width="90" align="center" column-key="NSOS_RECORD_TYPE" :formatter="formatDrop"></el-table-column>
    ...
    <h3>&nbsp;</h3>
    <h3 class="section-title">通知/催办详情</h3>
...
const ticketNumber = defineModel('ticketNumber')
const reportType = defineModel('reportType')
const notificationType = defineModel('notificationType')
const formData = reactive({
    ...
    const notificationForm = reactive({
      ...
      /* faultCode: formData.ticketNumber,
      reportType: formData.reportType,
      recordType: formData.notificationType, */
    });
    ...
    const fetchNotificationList = async () => {
      // 模拟从服务器获取通知列表
      // 这里可以替换为实际的 API 请求
      const response = await requestClient.post(import.meta.env.VITE_NSOSGZ + '/reportNotifyRecord/queryDataLists', {
        "faultCode": formData.ticketNumber,
        "reportType": 1
      });
    ...
    const add = async () => {
      ...
      const response = await requestClient.post(import.meta.env.VITE_NSOSGZ + '/reportNotifyRecord/add', parm);
