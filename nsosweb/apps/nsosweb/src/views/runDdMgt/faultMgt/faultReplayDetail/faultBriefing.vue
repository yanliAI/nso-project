<template>
  <div class="w-full h-full pt-2">
    ...
    <div class="mt-2">
      <AttachFileA :key="data.fileCompKey" ref="attachListRef" v-if="data.faultCode" :parentId="data.faultCode" :moduleId="'faultBriefing'" :showPreview="true" @afterUpload="afterUpload" @afterDelete="afterDelete">
    ...
    <!-- 通知 -->
    <ModalTz class="w-[1800px] h-[800px]" title="故障复盘>故障简报>通知信息" :footer="false" :closeOnClickModal="false" :draggable="true">
      <notify-or-urge record-type="1" module="3" sub-module="2" :busi-code="data.faultCode" tache-code="brief" tache-name="故障简报" :notify-target="tzcbData.userCode" :contact="tzcbData.userPhone" :content="tzcbData.content" :openAdd="tzcbData.openAdd" />
    </ModalTz>

    <!-- 催办 -->
    <ModalCb class="w-[1800px] h-[800px]" title="故障复盘>故障简报>催办信息" :footer="false" :closeOnClickModal="false" :draggable="true">
      <notify-or-urge record-type="2" module="3" sub-module="2" :busi-code="data.faultCode" tache-code="brief" tache-name="故障简报" :notify-target="tzcbData.userCode" :contact="tzcbData.userPhone" :content="tzcbData.content" :openAdd="tzcbData.openAdd" />
    </ModalCb>
  </div>
</template>
<script setup>
...
const queryData = async () => {
  const res = await queryReportEditByCode({ faultCode: formData.value.faultCode, reportType: '1' })
...
const saveData = async () => {
  await dataFormRef.value.validate(async (valid) => {
    if (valid) {
      try {
        await saveReportEdit(data.fmData, { params: { faultCode: formData.value.faultCode, reportType: '1' } })
        modalApiA.close()
        ElNotification({
          title: '提示',
          message: '保存成功！',
...
const addRowClick = async () => {
  data.fmData = await initReportEdit({ faultCode: formData.value.faultCode, reportType: '1' })
