  ...
  <div class="fill-whole">
    <el-tabs ref="myTab" v-model="data.activeTab" type="card" @tab-remove="removeTab" class="h-full w-full bg-white">
        ...
        <el-container class="h-full bg-white">
          <el-header height="50px">
            <el-form label-width="auto" :model="data.formData" :inline="true" label-position="right" class="demo-form-inline mt-2">
              <el-row>
                <el-col :span="5">
                  <el-form-item label="任务名称:" prop="dutyTaskName" style="width: 100%">
                    <el-input v-model="data.formData.dutyTaskName" clearable />
                  </el-form-item>
                </el-col>
          ...
          <el-main style="height: calc(100% - 95px)">
            ...
            >
              ...
              <el-table-column label="单号" align="center" width="120">
                <template #default="scope">
                  <a class="primary clickable" @click="showDetail(scope.row)">{{ scope.row.orderCode }}</a>
                </template>
              </el-table-column>
              <el-table-column prop="initiationTime" label="值班日期" :formatter="tDateFormat" align="center" width="200" />
              <el-table-column prop="dutyTaskName" label="值班任务" width="" />
              <el-table-column prop="creatorName" label="发起人" align="center" width="150" />
              <!-- <el-table-column label="流程状态" align="center" width="80">
                <template #default="scope">
                  <span v-if="scope.row.status === '1'" style="color: #ffa500">待确认</span>
                  <span v-if="scope.row.status === '2'" style="color: #32cd32">已确认</span>
                  <span v-else></span>
                </template>
              </el-table-column> -->
              <!-- <el-table-column prop="remark" label="变更信息" /> -->
              <el-table-column prop="deptName" label="填报单位" align="center" width="300" />
            </el-table>
          </el-main>
      ...
      <el-tab-pane v-for="item in data.tabArr" :key="item.name" :label="item.title" :name="item.name" closable>
        <component :is="item.comp" :orderCode="item.orderCode" :name="item.name" @refreshList="emit('refresh')" @removeTab="removeTab" />
      </el-tab-pane>
    </el-tabs>
  </div>
</template>
...
import { queryData } from './dutyConfirmMethod.ts'
import { tDateFormat, tDateTimeFormat, formatDrop, lookFlow } from '#/utils/commonUtil.ts'
import detailPage from './dutyConfirmDetail.vue'
...
onMounted(async () => {
  data.formData.status = props.status
})
const mytable = ref()
...
const query = async () => {
  ...
  data.formData.deptCode = userStore.userInfo.logincode === 'admin' ? null : userStore.userInfo.orgcode
  data.formData.status = props.status
  ...
  if (props.status === 1) {
    emit('amount', 'DQR', data.pageInfo.total)
  } else if (props.status === 2) {
    emit('amount', 'YQR', data.pageInfo.total)
  }
}
...
const showDetail = (row) => {
  ...
  if (row) {
    tabArr.some((item) => {
      if (item.orderCode == row.orderCode) {
        tab = item
        return true
      }
    })
  ...
  if (!tab) {
    ...
    if (row) {
      // 已保存工单tab信息
      tab.title = row.orderCode ? row.orderCode : '未编号'
      tab.orderCode = row.orderCode
    }
    tab.comp = detailPage
    tabArr.push(tab)
  }
