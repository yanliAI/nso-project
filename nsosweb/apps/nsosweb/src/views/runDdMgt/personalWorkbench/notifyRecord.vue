<template>
  ...
  <Modal class="w-[800px] h-[220px]" title="反馈信息" :footer="false" :closeOnClickModal="false" :draggable="true">
      ...
      <!-- <el-tab-pane v-for="item in data.tabArr" :key="item.name" :label="item.title" :name="item.name" closable>
        <DutyTaskDTB  :dtbid="item.id" :code='item.receiveDeptCode' @refresh="refresh" :tabName="item.name"/>
      </el-tab-pane> -->
    </el-tabs>
  </div>
</template>
...
//import DutyTaskDTB from '../dutyTaskFill/dutyTask-DTB.vue'
...
const emit = defineEmits(['updateTatol','queryTB'])
onMounted(async () => {
...
async function refresh(tabName) {
  await query();
  removeTab(tabName)
}
 ...
 */
 ...
 const edit = async (row) => {
  ...
  // modalApi.open()
  try{
    await requestClient.post(import.meta.env.VITE_NSOSGZ + '/notifyRecord/add2', row);
