  ...
  <el-container class="h-full bg-white">
    ...
    <el-main style="height: calc(100% - 80px);">
      <el-table :data="data.tableData" border  style="height: 100%;" stripe :header-cell-style="{ backgroundColor: '#F2F3F5', color: '#666666', fontSize: '14px', height: '40px', textAlign: 'center' }">
        ...
        <el-table-column prop="editRoleName" label="填报单位" width="200" align="center"></el-table-column>
        ...
        <el-table-column prop="reviewcontent" label="审核内容" width="150"  align="center"></el-table-column>
        <el-table-column prop="actFinishTime" label="处理时间" width="250" :formatter="tDateTimeFormat" align="center"></el-table-column>
        <el-table-column prop="processingresults" label="处理结果" width="150" align="center"></el-table-column>
      </el-table>
    </el-main>
    <el-footer height="40px">
      ...
      <el-tab-pane v-for="item in data.tabArr" :key="item.name" :label="item.title" :name="item.name" closable>
        <DutyTaskYSB  :dtbid="item.id" :code='item.receiveDeptCode' />
      </el-tab-pane>
    </el-tabs>
  </div>
</template>
 ...
 const showDetail = (row) => {
  ...
  if (!tab) {
    ...
    if (row) {
      // 已保存工单tab信息
      tab.title = '护网值班表填报'
      tab.id = row.faultCode
      tab.receiveDeptCode = row.editRole
    } else {
      // 新增工单tab信息
      tab.title = '新增'
      tab.id = null
