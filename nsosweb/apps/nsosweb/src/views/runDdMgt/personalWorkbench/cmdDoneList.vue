  ...
  <div class="w-full h-full">
    <el-tabs ref="myTab" v-model="data.tabIndex" @tab-remove="removeTab" type="card" @tab-change="tabChange" class="w-full h-full bg-white">
      <el-tab-pane label="全部记录" name="list" :closable="false">
        <el-container class="h-full bg-white">
          <el-header height="35px">
            <el-form label-width="auto" ref="ruleFormRef" :model="data.formData" size="small" :inline="true" label-position="right" class="demo-form-inline mt-1">
              <el-row>
                <el-col :span="4">
          ...
          <el-main style="height: calc(100% - 75px);">
            <el-table :data="data.tableData" border show-overflow-tooltip size="small" style="height: 100%;" stripe :header-cell-style="{ backgroundColor: '#F2F3F5', color: '#666666', fontSize: '14px', height: '40px', textAlign: 'center' }">
              ...
              <el-table-column prop="nodeName" label="操作环节" width="120" align="center"/>
              <el-table-column prop="commanderName" label="发令人" width="100" align="center"/>
              <el-table-column prop="flDeptName" label="发令单位" width="140" align="center"/>
      ...
      <el-tab-pane v-for="item in data.tabArr" :key="item.name" :label="item.title" :name="item.name" closable>
        <CmdDetail :cmdId="item.id" :oprateFlow="item.oprateFlow" :doneFlag="true" :tabName="item.name" @afterAdd="afterAdd($event, item)" @refreshList="refreshList" @removeTab="removeTab"></CmdDetail>
      </el-tab-pane>
    </el-tabs>
  </div>
</template>
...
import { queryDoneList, toExcel } from './cmdMgtMethod.ts'
...
const query = async (showmask) => {
  ...
  const { list, count, pageNo } = await queryDoneList(data.formData, {showmask: showmask})
  data.tableData = list
