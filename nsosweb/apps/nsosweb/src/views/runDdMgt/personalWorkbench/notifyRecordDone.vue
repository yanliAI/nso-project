  ...
  <el-container class="h-full bg-white">
    ...
    <el-main style="height: calc(100% - 80px);">
      <el-table :data="data.tableData" border  style="height: 100%;" stripe :header-cell-style="{ backgroundColor: '#F2F3F5', color: '#666666', fontSize: '14px', height: '40px', textAlign: 'center' }">
        ...
        <el-table-column prop="faultCode" label="单号/编号" width="220" align="center" header-align="center">
          <!-- <template #default="scope">
            <a @click="edit(scope.row)" class="my-text-link">
              {{ scope.row.faultCode }}
            </a>
          </template> -->
        </el-table-column>
        <el-table-column prop="taskcontent" label="标题/任务内容"   align="left" header-align="center">
          <!-- <template #default="scope">
            <a @click="edit(scope.row)" class="my-text-link">
        ...
        <el-table-column prop="reviewcontent" label="审核内容" width="140"  align="center"></el-table-column>
        <el-table-column prop="actFinishTime" label="处理时间" width="160" :formatter="tDateTimeFormat" align="center"></el-table-column>
        <el-table-column prop="processingresults" label="处理结果" width="140" align="center"></el-table-column>
      </el-table>
    </el-main>
...
<Modal class="w-[800px] h-[220px]" title="反馈信息" :footer="false" :closeOnClickModal="false" :draggable="true">
      <TemplateDiaLog :formData="data.selectedRow" :formData2="data.response" :flag="data.flag" @close="addClose" @closeDiaLog="closeDiaLog" @query="query"></TemplateDiaLog>
    </Modal>
    </el-tabs>
  </div>
</template>
...
import TemplateDiaLog from './TemplateDiaLogA.vue'
import DutyTaskYSB from '../dutyTaskFill/dutyTask-YSB.vue'
...
onMounted(async () => {
  ...
  const currentDate = new Date(); // 当前时间（本地时区）
  currentDate.setHours(23,59,59,999);
  const nextMonthDate = new Date(currentDate);
  nextMonthDate.setMonth(currentDate.getMonth() - 1);
  nextMonthDate.setHours(0,0,0,0);
  console.log(formatDateTime(currentDate));
  data.formData.timeRange[0] = formatDateTime(nextMonthDate);
  data.formData.timeRange[1] = formatDateTime(currentDate);
  query()
  queryCount();
});
...
const query = async () => {
  ...
  if (data.formData.timeRange) {
    data.formData.startTime1 = data.formData.timeRange[0]
    data.formData.endTime1 = data.formData.timeRange[1]
  }else {
    data.formData.startTime1 = null;
    data.formData.endTime1 = null;
  }
  data.formData.editState = '2';
  data.formData.businesstype = 6;
 ...
 */
 ...
 const edit = async (row) => {
  ...
  const response = await requestClient.post(import.meta.env.VITE_NSOSGZ + '/notifyRecord/query', parm);
  data.selectedRow = row
  data.response = response
  data.flag.flag = row.processingresults === '已反馈' ? true : false;
