  ...
  <div class="w-full h-full">
    <el-tabs ref="myTab" v-model="data.tabIndex" @tab-remove="removeTab" type="card" @tab-change="tabChange" class="w-full h-full bg-white">
      <el-tab-pane label="全部记录" name="list" :closable="false">
        <el-container class="h-full bg-white">
          ...
          <el-main style="height: calc(100% - 75px);">
            <el-table :data="data.tableData" border show-overflow-tooltip size="small" style="height: 100%;" stripe :header-cell-style="{ backgroundColor: '#F2F3F5', color: '#666666', fontSize: '14px', height: '40px', textAlign: 'center' }">
              ...
              <el-table-column prop="commandTitle" label="名称" align="left"/>
              <el-table-column prop="wcTime" label="完成时限" :formatter="tDateTimeFormat" width="150" align="center"/>
              <el-table-column v-if="queryState == '1'" header-align="center" width="90" label="是否超时签收" align="center">
                <template #default="scope">
                  <span v-if="(data.serverTime - scope.row.flTime) > 2*60*60*1000" class="text-red-600">是</span>
                  <span v-else>否</span>
                </template>
              </el-table-column>
              <el-table-column v-if="queryState == '2'" header-align="center" width="90" label="是否超时复令" align="center">
                <template #default="scope">
              ...
              <el-table-column prop="commander" label="发令人" width="100" align="center"/>
              <el-table-column prop="flDept" label="发令单位" width="140" align="center"/>
      ...
      <el-tab-pane v-for="item in data.tabArr" :key="item.name" :label="item.title" :name="item.name" closable>
        <CmdDetail :cmdId="item.id" :oprateFlow="item.oprateFlow" :tabName="item.name" @afterAdd="afterAdd($event, item)" @refreshList="refreshList" @removeTab="removeTab"></CmdDetail>
      </el-tab-pane>
    </el-tabs>
  </div>
</template>
...
onMounted(async () => {
  ...
  const dropType = ['NSOS_COMMAND_STATE_SL','NSOS_ORG_NAME_SLDW', 'NSOS_CMD_TYPE', 'NSOS_NOTICE_TYPE', 'NSOS_CMD_FL']
...
const query = async (showmask) => {
  ...
  const { list, count, pageNo, serverTime } = await queryTodoList(data.formData, {showmask: showmask})
  data.tableData = list
  data.pageInfo.pageNo = pageNo;
  data.pageInfo.total = count;
  data.serverTime = serverTime
  emit('updateTatol', queryState.value - 2, count)
  formatEntity()
  refreshCmdType()
}
