...
const { appendToMain, bordered, cancelText, centered, class: modalClass, closable, closeOnClickModal, closeOnPressEscape, confirmDisabled, confirmLoading, confirmText, contentClass, darkCss, description, draggable, footer: showFooter, footerClass, fullscreen, fullscreenButton, header, headerClass, loading: showLoading, modal, openAutoFocus, showCancelButton, showConfirmButton, title, titleTooltip, zIndex } = usePriorityValues(props, state)
...
<template>
  <Dialog :modal="false" :open="state?.isOpen" @update:open="() => modalApi?.close()">
    <DialogContent
      ...
      :show-close="closable"
      :z-index="zIndex"
      :close-class="darkCss ? 'top-2 text-[#fff]/100 opacity-80' : 'top-2'"
    ...
    >
      <DialogHeader
        ...
        "
      >
        <DialogTitle v-if="title" class="text-left text-[15px]">
          <slot name="title">
            {{ title }}
      ...
      <div
        ref="wrapperRef"
        :class="
          cn('relative min-h-40 flex-1 overflow-y-auto px-3 py-1', contentClass, {
      ...
      <VbenIconButton
        ...
        :class="
          darkCss == true
            ? 'hover:bg-accent hover:text-accent-foreground text-[#fff]/100 flex-center absolute right-10 top-2 hidden size-6 rounded-full px-1 text-lg opacity-80 transition-opacity hover:opacity-100 focus:outline-none disabled:pointer-events-none sm:block'
            : 'hover:bg-accent hover:text-accent-foreground text-foreground/80 flex-center absolute right-10 top-2 hidden size-6 rounded-full px-1 text-lg opacity-70 transition-opacity hover:opacity-100 focus:outline-none disabled:pointer-events-none sm:block'
        "
      ...
      >
        ...
        <slot name="footer">
          ...
          <component :is="components.PrimaryButton || VbenButton" v-if="showConfirmButton" :disabled="confirmDisabled" :loading="confirmLoading" @click="() => modalApi?.onConfirm()">
